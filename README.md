...
Input and output contexts are used to maintain conversational flow across multiple user messages. This allows the chatbot to remember previously added items and continue the order seamlessly until the user confirms completion.
...
When users do not specify a quantity, the chatbot intelligently defaults the quantity to one instead of interrupting the conversation. This improves user experience and reflects real-world food ordering behavior.
